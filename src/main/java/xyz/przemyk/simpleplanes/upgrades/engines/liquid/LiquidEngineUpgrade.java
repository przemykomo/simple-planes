    ...
    public void tick() {
        if (!planeEntity.level.isClientSide) {
            if (burnTime > 0) {
                ...
                updateClient();
            } else if (planeEntity.getThrottle() > 0 && !fluidTank.isEmpty() && PlaneLiquidFuelReloadListener.fuelMap.getOrDefault(fluidTank.getFluid().getFluid(), 0) > 0) {
                fuelBurned = -burnTime / PlaneLiquidFuelReloadListener.fuelMap.getOrDefault(fluidTank.getFluid().getFluid(), 0) + 1;
                burnTime = fuelBurned * PlaneLiquidFuelReloadListener.fuelMap.getOrDefault(fluidTank.getFluid().getFluid(), 0);
                if (burnTime > 0) {
