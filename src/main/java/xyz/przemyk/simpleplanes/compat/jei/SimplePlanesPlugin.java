...
public class SimplePlanesPlugin implements IModPlugin {
    ...
    public ResourceLocation getPluginUid() {
        return ResourceLocation.fromNamespaceAndPath(SimplePlanesMod.MODID, "jei_plugin");
    }
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        registration.addRecipes(PlaneWorkbenchRecipeCategory.RECIPE_TYPE,
            Minecraft.getInstance().level.getRecipeManager()
                .getAllRecipesFor(SimplePlanesRecipes.PLANE_WORKBENCH_RECIPE_TYPE.get()).stream().map(RecipeHolder::value).toList());
    }
