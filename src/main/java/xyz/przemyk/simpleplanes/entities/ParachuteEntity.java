...
public class ParachuteEntity extends Entity implements ImplementedInventory {
    ...
    public ParachuteEntity(Level level) {
        super(SimplePlanesEntities.PARACHUTE, level);
    }
    ...
    public ParachuteEntity(Level level, NonNullList<ItemStack> items) {
        super(SimplePlanesEntities.PARACHUTE, level);
    ...
    public void tick() {
        ...
        if ((passenger == null && !hasStorageCrate()) || !level.getBlockState(new BlockPos(getX(), getY() - 0.1, getZ())).getMaterial().isReplaceable()) {
            kill();
            spawnAtLocation(SimplePlanesItems.PARACHUTE_ITEM);
                ...
                for (int i = 0; i < 50; i++) {
                    ...
                    if (blockState.getMaterial().isReplaceable()) {
                        ...
                        if (level.getBlockEntity(mutableBlockPos) instanceof BarrelBlockEntity barrelBlockEntity) {
                            for (int s = 0; s < 27; s++) {
                                ItemStack itemStack = items.get(s);
    ...
    protected void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (hasStorageCrate()) {
            ContainerHelper.loadAllItems(compoundTag.getCompound("items"), items);
        }
    }
    ...
    protected void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (hasStorageCrate()) {
            compoundTag.put("items", ContainerHelper.saveAllItems(new CompoundTag(), items));
        }
    }
