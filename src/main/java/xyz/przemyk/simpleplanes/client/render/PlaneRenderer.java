...
public class PlaneRenderer<T extends PlaneEntity> extends EntityRenderer<T> {
    ...
    public static ResourceLocation getMaterialTexture(PlaneEntity entity) {
        ...
        try {
            ResourceLocation sprite = Minecraft.getInstance().getModelManager().getModel(new ModelResourceLocation(Registry.BLOCK.getKey(block), "inventory")).getQuads(null, Direction.SOUTH, RandomSource.create()).get(0).getSprite().getName();
